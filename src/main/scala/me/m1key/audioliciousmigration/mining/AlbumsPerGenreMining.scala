...
class AlbumsPerGenreMining @Inject() (private val persistenceProvider: MorphiaMongoDbPersistenceProvider) {
  ...
  def mine(): Option[List[(String, Int)]] = {
  ...
  private def processAndRetrieveResults(list: BasicDBList): List[(String, Int)] = {
  ...
  private def processResult(dbObject: BasicDBObject): (String, Int) = {
    val artistName = dbObject.get("genre").toString()
