...
class MostPlayedAlbumsPerSongMining @Inject() (private val persistenceProvider: MorphiaMongoDbPersistenceProvider) {
  ...
  private val query = "db.MongoDbSong.group(" +
	...
	"reduce: function(obj, prev) {obj.statsList.forEach(function(entry){if(entry.libraryUuid == '%s' && ++prev.totalSongs){ prev.totalPlays += entry.playCount}})}," +
  ...
  private def processResult(dbObject: BasicDBObject): (String, String, Double) = {
    ...
    val totalPlays = parseDouble(dbObject.get("totalPlaysPerSong").toString(), formatter)
