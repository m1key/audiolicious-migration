...
class SongSkipCountMining @Inject() (private val persistenceProvider: MorphiaMongoDbPersistenceProvider) {
  ...
  def mine(limit: Int, libraryUuid: String): List[MongoDbSong] = {
    val query = persistenceProvider.getDatastore().createQuery(classOf[MongoDbSong]).order("-statsList.skipCount").filter("statsList.libraryUuid", libraryUuid).limit(limit);
