...
class HighestRatedAlbumMining @Inject() (private val persistenceProvider: MorphiaMongoDbPersistenceProvider) {
  ...
  private def processResult(dbObject: BasicDBObject): (String, String, Double) = {
    ...
    val totalPlays = parseDouble(dbObject.get("totalRatingPerAlbum").toString(), formatter)
